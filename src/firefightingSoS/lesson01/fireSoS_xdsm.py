...
from aircraft_sizing.examples.ex_01_asw.viz import xdsm


DIAGRAM_BASENAME = "fireSoS_xdsm"
...
def build_xdsm():
    ...
    xdsm.add_system("opt", SOLVER, (r"\text{Optimizer}",))
    xdsm.add_system("aircraft", FUNC, (r"\text{Fixed Aircraft}",))
    xdsm.add_system("perf", FUNC, (r"\text{Aircraft Performance}",))
    xdsm.add_system("sim", FUNC, (r"\text{Simulation}",))

    xdsm.add_input("opt", r"\text{Budget}")
    xdsm.connect("opt", "aircraft", r"\text{Fleet Composition}")
    xdsm.connect("opt", "perf", r"\text{Cruise Dist., Loiter Time, Payload size, Sortie count}")
    xdsm.connect("opt", "sim", (r"\text{tactics, T/O and land location}"))

    # Feed-forward couplings (upper triangle).
    xdsm.connect("aircraft", "perf", r"\text{Fuel cap, MTOW, T/O and Land distances, TSFC, Payload cap}")
    xdsm.connect("perf", "sim", r"\text{Fuel burn, Refuel rate, Reload rate}")

    xdsm.add_output("sim", r"\text{Mission}", side=RIGHT)
    xdsm.add_output("perf", r"\text{Operating Ceiling, Max Range, Max Endurance}", side=RIGHT)
    xdsm.connect("sim", "opt", r"\text{MoE}")

    xdsm.add_process(["opt", "aircraft", "perf", "sim", "opt"], arrow=True)
